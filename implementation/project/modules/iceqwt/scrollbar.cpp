...
ScrollBar::ScrollBar(double minBase, double maxBase, Qt::Orientation orientation,
	QWidget* parent) : QScrollBar(orientation, parent)
{
	init();
...
{
	const int sliderTicks = qRound((max - min) / (m_maxBase - m_minBase) * m_baseTicks);
